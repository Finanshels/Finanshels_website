...
export default function ServiceDetailPage({ page }) {
  ...
  return (
    <div className="bg-white text-slate-900">
      <section className="pt-36 pb-16 px-6 sm:px-10 lg:px-16 bg-gradient-to-b from-[#0b1324] via-[#0f172a] to-[#141b2f] text-white relative overflow-hidden">
        <div className="absolute inset-0 opacity-20 bg-[radial-gradient(circle_at_top,_rgba(241,102,16,0.4),_transparent_60%)]" />
        <div className="max-w-6xl mx-auto relative z-10 space-y-8">
          <AnimatedSection animation="fade-down">
            <p className="text-xs uppercase tracking-[0.4em] text-white/60 font-semibold">{page.title}</p>
            <h1 className="text-4xl sm:text-5xl font-semibold leading-tight max-w-4xl">{page.subtitle}</h1>
            <p className="text-lg text-white/80 max-w-3xl mt-6">{page.description}</p>
          </AnimatedSection>
          {page.stats && (
      ...
      <section className="py-16 px-6 sm:px-10 lg:px-16 bg-white">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-8">
          <AnimatedSection animation="fade-right">
            <Card className="h-full border border-slate-100 rounded-[32px] p-8">
              <p className="text-xs uppercase tracking-[0.4em] text-[#f16610]/80 font-semibold">Signals you feel</p>
              <div className="mt-6 space-y-4">
                {signals.map((signal, index) => (
                  <div key={signal} className="flex gap-4">
                    <div className="w-11 h-11 rounded-2xl bg-[#fff4ec] text-[#f16610] flex items-center justify-center font-semibold">
          ...
          <AnimatedSection animation="fade-left">
            <Card className="h-full border border-slate-100 rounded-[32px] p-8 bg-[#f8fafc]">
              <p className="text-xs uppercase tracking-[0.4em] text-[#f16610]/80 font-semibold">Why teams call Finanshels now</p>
              <div className="mt-6 space-y-3">
                {triggers.map((trigger) => (
                  <div key={trigger} className="rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-700">
                    {trigger}
                  </div>
                ))}
              </div>
      ...
      <section className="py-16 px-6 sm:px-10 lg:px-16 bg-[#0f172a] text-white">
        <div className="max-w-6xl mx-auto space-y-10">
          <AnimatedSection animation="fade-up">
            <div className="space-y-3">
              <p className="text-xs uppercase tracking-[0.4em] text-white/70 font-semibold">Story arc</p>
              <h2 className="text-3xl font-semibold">How the engagement unfolds</h2>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {storyBeats.map((beat, index) => (
              <AnimatedSection key={beat.phase} animation="fade-up" delay={index * 60}>
                <Card className="h-full rounded-[30px] border border-white/20 bg-white/5 p-6 space-y-3">
                  <p className="text-xs uppercase tracking-[0.4em] text-white/70 font-semibold">{beat.phase}</p>
                  <h3 className="text-xl font-semibold">{beat.headline}</h3>
                  <p className="text-white/80 text-sm leading-relaxed">{beat.description}</p>
                </Card>
              </AnimatedSection>
            ))}
          </div>
      ...
      <section className="py-16 px-6 sm:px-10 lg:px-16 bg-[#f8fafc]">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-10">
          <AnimatedSection animation="fade-right">
            <Card className="h-full rounded-[32px] border border-slate-100 bg-white p-8 space-y-4">
              <p className="text-xs uppercase tracking-[0.4em] text-[#f16610]/80 font-semibold">Why Finanshels</p>
              <div className="space-y-3">
                {page.valueProps?.map((prop) => (
                  <div key={prop} className="rounded-2xl bg-[#fff7f1] p-4 text-sm text-slate-700">
                    {prop}
                  </div>
                ))}
              </div>
          ...
          <AnimatedSection animation="fade-left">
            <Card className="h-full rounded-[32px] border border-slate-100 bg-white p-8 space-y-4">
              <p className="text-xs uppercase tracking-[0.4em] text-[#f16610]/80 font-semibold">Solution blueprint</p>
              <div className="space-y-4">
                {blueprint.map((item) => (
                  <div key={item} className="flex gap-3">
                    <Sparkles className="text-[#f16610]" size={20} />
      ...
      <section className="py-16 px-6 sm:px-10 lg:px-16 bg-white">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-10">
          <AnimatedSection animation="fade-right">
            <Card className="h-full rounded-[32px] border border-slate-100 bg-white p-8 space-y-4">
              <p className="text-xs uppercase tracking-[0.4em] text-[#f16610]/80 font-semibold">Execution rituals</p>
              <div className="space-y-4">
                {rituals.map((step, index) => (
                  <div key={step} className="flex gap-4">
                    <div className="w-10 h-10 rounded-2xl bg-[#fff4ec] text-[#f16610] flex items-center justify-center font-semibold">
                      {(index + 1).toString().padStart(2, '0')}
                    </div>
          ...
          <AnimatedSection animation="fade-left">
            <Card className="h-full rounded-[32px] border border-slate-100 bg-[#f8fafc] p-8 space-y-4">
              <p className="text-xs uppercase tracking-[0.4em] text-[#f16610]/80 font-semibold">What leadership receives</p>
              <div className="space-y-3">
                {deliverables.map((deliverable) => (
                  <div key={deliverable} className="flex gap-3 text-sm text-slate-700 rounded-2xl border border-slate-200 bg-white p-4">
      ...
      {page.caseStudy && (
        <section className="py-16 px-6 sm:px-10 lg:px-16 bg-slate-950 text-white">
          <div className="max-w-5xl mx-auto">
            <AnimatedSection animation="fade-up">
              <Card className="bg-white/10 border-white/20 rounded-[36px]">
                <div className="p-10 space-y-4">
                  <p className="text-xs uppercase tracking-[0.4em] text-white/70 font-semibold">Case study</p>
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-8">
          {testimonials.map((testimonial, index) => (
            <AnimatedSection key={testimonial.name} animation="fade-up" delay={index * 80}>
              <Card className="h-full border border-slate-100 rounded-[32px] p-8 space-y-4">
      ...
      <section className="py-16 px-6 sm:px-10 lg:px-16 bg-[#fff8f2]">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-8">
          <AnimatedSection animation="fade-right">
            <Card className="h-full border border-[#ffd7c0] rounded-[36px] bg-white">
              <div className="p-8 space-y-4">
                <p className="text-xs uppercase tracking-[0.4em] text-[#f16610]/80 font-semibold">Subscription snapshot</p>
          ...
          <AnimatedSection animation="fade-left">
            <Card className="h-full border border-[#ffd7c0] rounded-[36px] bg-white">
              <div className="p-8 space-y-4">
                <p className="text-xs uppercase tracking-[0.4em] text-[#f16610]/80 font-semibold">Request a working session</p>
                <form className="space-y-4" onSubmit={handleSubmit}>
                  ...
                  <textarea
                    ...
                    className="w-full rounded-2xl border border-slate-200 px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#f16610]/30"
                  />
                  <button type="submit" className="w-full rounded-2xl bg-[#f16610] px-5 py-3 font-semibold text-white">
                    {submitted ? 'Got it — reply coming within 24 hrs.' : 'Book a strategy call'}
                  </button>
                </form>
                <p className="text-xs text-slate-500">
                  Prefer a direct line? <a href="mailto:hello@finanshels.com" className="text-[#f16610] font-semibold">hello@finanshels.com</a> or{' '}
